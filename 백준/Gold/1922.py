...
def main():
    ...
    for _ in range(e):
        ...
        adj_nodes[v1].append((v2, w))
        adj_nodes[v2] = adj_nodes.get(v2, [])
        adj_nodes[v2].append((v1, w))

    # print(adj_nodes)

    start_node = 1
    priority_queue = deque([(start_node, 0)])
    visited = [False for _ in range(v + 1)]
    # visited[start_node] = True

    total_weight = 0
    while priority_queue:
        node, weight = remove_from_heap(priority_queue)
        # print(node, weight)
        if visited[node]:
            continue
        visited[node] = True
        ...
        for n, w in adj_nodes[node]:
            if not visited[n]:
                add_to_heap(priority_queue, n, w)
